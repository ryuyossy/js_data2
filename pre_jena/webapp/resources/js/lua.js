jQuery(function($) {
	...
	luaLenght.forEach(function(element) {
	...
	$('#changeMeSubm_1').on('click', function() {
		...
		if (!first && !last) {
			...
			U.removeLocalStorage('lua_' + currentKanteiId + '_user');
		} else {
			U.saveLocalStorage('lua_' + currentKanteiId + '_user', {
				'firstName': first,
				'lastName': last,
				'birthday': birthday
			});
		...
		U.touchButton(idType, function(target) {
			...
			if (userTickets[_ticketType] >= _ticketCount) {
				...
				U.doQuestionFormSettings(
						...
						strButtonAfter,
						strButtonYes,
						'/report/12/menu/' + $this.attr('data-menu-id'),
						$('#rtoken').text(),
						$this.attr('data-role'),
						_ticketType,
						_ticketCost,
	...
	(function() {
		...
		setUserInfo($('#menu1Button'), 'lua_menu1_user', '.js_kanaFullNameMe_menu1', '.js_fullbirthMe_menu1');
		setUserInfo($('#menu2Button'), 'lua_menu2_user', '.js_kanaFullNameMe_menu2', '.js_fullbirthMe_menu2');
		setUserInfo($('#menu3Button'), 'lua_menu3_user', '.js_kanaFullNameMe_menu3', '.js_fullbirthMe_menu3');
