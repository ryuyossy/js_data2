jQuery(function($) {
	...
	var changeFormSubm = function(idType, boolType, stgType) {
		$(idType).on('click', function() {
			...
			if (!first && !last && !firstKanji && !lastKanji) {
				...
				U.removeLocalStorage('jena_' + currentKanteiId + '_target');
			} else {
				U.saveLocalStorage('jena_' + currentKanteiId + stgType, {
					'firstName': first,
					'lastName': last,
		...
		U.touchButton(idType, function(target) {
			...
			if (userTickets[_ticketType] >= _ticketCount) {
				...
				U.doQuestionFormWithSet1(
						...
						strButtonAfter,
						strButtonYes,
						'/report/16/menu/' + $this.attr('data-menu-id'),
						$('#rtoken').text(),
						$this.attr('data-role'),
						_ticketType,
						_ticketCost,
	...
	(function() {
		...
		function setInfo() {
			menuType.forEach(function(elem) {
				setUserInfo($('#' + elem + '_Button'), 'jena_' + elem + '_user', '.js_kanjiFullNameMe_' + elem, '.js_kanaFullNameMe_' + elem, '.js_fullbirthMe_' + elem, true);
				setUserInfo($('#' + elem + '_Button'), 'jena_' + elem + '_target', '.js_kanjiFullNameYou_' + elem, '.js_kanaFullNameYou_' + elem, '.js_fullbirthYou_' + elem, false);
			});
		}
