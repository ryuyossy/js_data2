jQuery(function($) {
	...
	var changeFormSubm = function(idType, boolType, stgType) {
		$(idType).on('click', function() {
			...
			if (!first && !last && !firstKanji && !lastKanji) {
				...
				U.removeLocalStorage('klara_midnight_' + currentKanteiId + '_target');
			} else {
				U.saveLocalStorage('klara_midnight_' + currentKanteiId + stgType, {
					'firstName': first,
					'lastName': last,
		...
		U.touchButton(idType, function(target) {
			...
			if (userTickets[_ticketType] >= _ticketCount) {
				...
				U.doQuestionFormWithSet1(
						...
						strButtonAfter,
						strButtonYes,
						'/report/20/menu/' + $this.attr('data-menu-id'),
						$('#rtoken').text(),
						$this.attr('data-role'),
						_ticketType,
						_ticketCost,
	...
	var settings = function() {
		...
		menuType.forEach(function(elem) {
			setBtnPosts('#' + elem + '_Button', '.js_' + elem + '_NameCheck', false);
		});
	};
	...
	(function() {
		...
		function setInfo() {
			menuType.forEach(function(elem) {
				setUserInfo($('#' + elem + '_Button'), 'klara_midnight_' + elem + '_user', '.js_kanjiFullNameMe_' + elem, '.js_kanaFullNameMe_' + elem, '.js_fullbirthMe_' + elem, true);
				setUserInfo($('#' + elem + '_Button'), 'klara_midnight_' + elem + '_target', '.js_kanjiFullNameYou_' + elem, '.js_kanaFullNameYou_' + elem, '.js_fullbirthYou_' + elem, false);
			});
		}
